...
def main(config):
    ...
    unseen_log = '\n' + '-' * 30 + '\nUnseen Classes\n'
    for idx in get_unseen_idx(n_class):
        unseen_log += '\t%02d - %s\n' % (int(idx), VOC[int(idx)])
    logger.info(unseen_log)

    seen_log = '\nSeen Classes\n'
    for idx in get_seen_idx(n_class, ignore_bg=ignore_bg):
        seen_log += '\t%02d - %s\n' % (int(idx), VOC[int(idx)])
    seen_log += '-' * 30 + '\n'
    ...
    evaluator = config.init_obj(
        'evaluator',
        module_metric,
        *[dataset.num_classes, get_seen_idx(n_class, ignore_bg=ignore_bg), get_unseen_idx(n_class)]
    )
