...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        seen_idx_list = self.seen_classes_idx[:]
        if 0 in seen_idx_list:
            seen_idx_list.remove(0)
        self.semantic_relation_l2_seen = torch.cdist(
            torch.index_select(self.embeddings, 0, torch.Tensor(seen_idx_list).long().to(self.device)),
            torch.index_select(self.embeddings, 0, torch.Tensor(seen_idx_list).long().to(self.device)),
            p=2
        )
        self.semantic_relation_seen = F.softmax(-self.semantic_relation_l2_seen * self.TEMP, dim=-1)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(tqdm.tqdm(self.train_loader)):
            ...
            with torch.autograd.set_detect_anomaly(True):
                for i, s_class in enumerate(seen_idx_list):
                    proto_relation_l2_seen = torch.sqrt(((proto_embeddings[s_class].reshape(1, -1) - proto_embeddings[seen_idx_list])**2).sum(-1) + 1e-8)
                    proto_relation_seen = F.log_softmax(-proto_relation_l2_seen, dim=-1)
